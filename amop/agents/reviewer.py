...
class ReviewerAgent(BaseAgent):
    ...
    def system_prompt(self) -> str:
        return (
            ...
            "files, you MUST also fill in `counterexample`: a concrete "
            "INPUT -> OUTPUT pair where the diff produces the WRONG "
            "result (see its schema description below for the exact "
            "format). This is mechanically enforced -- a rejection of a "
            "passing, in-scope diff with no counterexample is "
            "automatically converted to an approval before a human ever "
            "sees it, regardless of what you set `approved` to. If you "
            "genuinely cannot construct one, the diff is not "
            "demonstrably wrong and the correct verdict is approved=true.\n\n"
